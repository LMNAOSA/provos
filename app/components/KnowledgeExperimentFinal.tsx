...
const DEMO: Case = {
  ...
  images: [
    { src: "/images/case001-uv-photo.svg", label: "365 NM UV", note: "Hard matrix under UV excitation · visible afterglow observed for ~7 seconds" },
    { src: "/images/case001-normal-photo.svg", label: "NORMAL LIGHT", note: "Same material under ordinary light" },
  ],
...
function hydrate(raw: string | null): Case[] {
  ...
  try {
    ...
    const found = out.some((c) => c.id === DEMO.id);
    ...
    return found ? normalized : [DEMO, ...normalized];
  } catch { return [DEMO]; }
}
...
export function KnowledgeExperiment() {
  ...
  const [activeId] = useState("CASE 001");
  ...
  const active = useMemo(() => cases.find((c) => c.id === activeId) || DEMO, [cases, activeId]);
  ...
  const update = (fn: (c: Case) => Case) => persist(cases.map((c) => c.id === activeId ? fn(c) : c));
  ...
  const returnEvidence = () => {
    ...
    update((c) => ({
      ...
      requests: c.requests.map((r) => r.id === openRequest.id ? { ...r, status: "FULFILLED", response: pxrfResponse.trim() } : r),
      answers: { ...c.answers, [openRequest.from]: { ...c.answers[openRequest.from]!, request: { ...openRequest, status: "FULFILLED", response: pxrfResponse.trim() } } },
      history: [...c.history, `pXRF RETURNED · REQUEST ${openRequest.id}`],
    }));
    setPxrfResponse("");
  };
  ...
  const lockReassessment = (expert: ExpertRole) => {
    ...
    if (!current || !current.request || current.request.status !== "FULFILLED" || !reassessment.trim()) return;
    ...
    <div className="experimentShell">
      <header className="experimentHeader">
        ...
        <div className="roleSwitch" aria-label="Demo participant">
          ...
          {(["MAT", "SPOONER", "DANIELLE"] as Role[]).map((r) => <button key={r} className={role === r ? "active" : ""} onClick={() => setRole(r)}>{PEOPLE[r].tag}</button>)}
        </div>
      </header>
      ...
      <section className="questionBlock">
        ...
        <div className="case001Images">
          {active.images.map((img) => <figure className="case001Image" key={img.src}><div className="imageFrame"><img className="fieldImage" src={img.src} alt={img.label} /></div><figcaption><span>{img.label}</span><small>{img.note}</small></figcaption></figure>)}
        </div>
      ...
      {role === "MAT" ? (
        <section className="matWaiting">
          <span className="kicker">FIELD · MATT KATHAGEN</span>
          <h3>Watch the blind readings take shape.</h3>
          <p>The field role supplies the observation and returns measurement data only after both independent initial judgements are sealed.</p>
          {openRequest ? <><div className="requestQuote">“{openRequest.prompt}”</div>{!bothLocked ? <div className="waitingMark">WAITING FOR THE SECOND INITIAL READING</div> : <><label className="kicker" htmlFor="pxrf">RETURN pXRF MEASUREMENT</label><textarea id="pxrf" value={pxrfResponse} onChange={(e) => setPxrfResponse(e.target.value)} placeholder="Enter the returned measurement exactly as supplied." /><button className="loginBtn" onClick={returnEvidence}>RETURN pXRF DATA <span>→</span></button></>}</> : <div className="waitingMark">NO OPEN EVIDENCE REQUEST</div>}
        </section>
      ) : (
        <section className="workflowStack">
          <section className="expertResponse">
            <div className="responseHead"><div><span className="kicker">{PEOPLE[role].tag} · INDEPENDENT RESPONSE</span><h3>{PEOPLE[role].name}</h3><p>{PEOPLE[role].title}</p></div><div className="blindLock">SEALED<br /><small>OTHER RESPONSE HIDDEN</small></div></div>
