...
export function KnowledgeExperiment() {
  ...
  return <div className="experimentShell">
    <section className="participantBar" aria-label="Choose demo participant">
      <div className="participantPrompt"><span className="kicker">CHOOSE YOUR ROLE</span><p>This is one case viewed from three positions. Select a participant before you begin.</p></div>
      <div className="participantOptions">
        {(Object.keys(PEOPLE) as Role[]).map((r) => <button key={r} className={role === r ? "active" : ""} onClick={() => setRole(r)}><span>{PEOPLE[r].name}</span><small>{r === "MAT" ? "FIELD · SUPPLIES OBSERVATION + MEASUREMENT" : r === "SPOONER" ? "SCIENCE · INDEPENDENT JUDGEMENT" : "ANALYSIS · INDEPENDENT JUDGEMENT"}</small></button>)}
      </div>
...
function History({ active }: { active: Case }) { return <section className="history"><span className="kicker">EVENT HISTORY · WHAT THE SYSTEM KEEPS</span><div>{active.history.map((item, i) => <p key={`${item}-${i}`}><span>{String(i + 1).padStart(2, "0")}</span><span>{item}</span></p>)}</div></section>; }
